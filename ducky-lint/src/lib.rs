...
fn default_suspicious_delay_threshold() -> u32 {
    10000
}
...
impl DuckyLinter {
    ...
    pub fn lint(&self, source: &str) -> Vec<LintIssue> {
        ...
        for (line_idx, line) in source.lines().enumerate() {
            ...
            if self.config.check_mixed_indentation && line.starts_with(' ') && line.contains('\t') {
                issues.push(LintIssue {
                    line: line_num,
                    column: 0,
                    severity: LintSeverity::Warning,
            ...
            if self.config.check_suspicious_delays
                && let Some(delay_str) = line.trim().strip_prefix("DELAY ")
                && let Ok(delay) = delay_str.trim().parse::<u32>()
                && delay > self.config.suspicious_delay_threshold
            {
                issues.push(LintIssue {
                    line: line_num,
                    column: 0,
