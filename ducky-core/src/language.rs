...
fn parse_codes(codes: &str) -> ([u8; 3], usize) {
    let mut parts = [0u8; 3];
    let mut count = 0;
    for p in codes.split(',') {
        if count < 3 {
            parts[count] = u8::from_str_radix(p, 16).unwrap_or(0);
        }
        count += 1;
    }
    (parts, count)
...
static DEFAULT_PARSED: OnceLock<HashMap<String, [u8; 3]>> = OnceLock::new();
static DEFAULT_ASCII: OnceLock<[Option<[u8; 3]>; 128]> = OnceLock::new();

fn default_parsed() -> &'static HashMap<String, [u8; 3]> {
    DEFAULT_PARSED.get_or_init(|| {
        default_string_map()
            .into_iter()
            .map(|(k, v)| (k, parse_codes(&v).0))
            .collect()
    })
}

fn default_ascii() -> &'static [Option<[u8; 3]>; 128] {
    DEFAULT_ASCII.get_or_init(|| {
        let mut table = [None; 128];
        for (k, v) in default_string_map() {
            let bytes = k.as_bytes();
            if bytes.len() == 1 && bytes[0] < 128 {
                table[bytes[0] as usize] = Some(parse_codes(&v).0);
            }
        }
        table
    })
...
impl KeyboardLayout {
    ...
    pub fn get_key_codes(&self, key: &str) -> Option<([u8; 3], usize)> {
        if self.is_default {
            let bytes = key.as_bytes();
            if bytes.len() == 1 && bytes[0] < 128 {
                return default_ascii()[bytes[0] as usize].map(|b| (b, 3));
            }
            default_parsed().get(key).map(|b| (*b, 3))
        } else {
            self.keys.get(key).map(|codes| parse_codes(codes))
        }
    }
