...
pub fn find_references(content: &str, position: Position, include_declaration: bool, uri: &Url) -> Vec<Location> {
        ...
        if let Some(symbol) = extract_symbol_at_position(line, position.character as usize) {
            ...
            for (line_idx, line) in content.lines().enumerate() {
                ...
                while let Some(pos) = line[search_pos..].find(&symbol) {
                    ...
                    let is_start = actual_pos == 0 || !line.chars().nth(actual_pos - 1).unwrap_or(' ').is_alphanumeric();
                    let is_end = actual_pos + symbol.len() >= line.len() 
                        || !line.chars().nth(actual_pos + symbol.len()).unwrap_or(' ').is_alphanumeric();
