...
pub fn token_type_to_semantic(token: &TokenType) -> (SemanticTokenType, Vec<SemanticTokenModifier>) {
    match token {
        ...
        // Keywords - Functions
        TokenType::Function | TokenType::EndFunction | TokenType::Return | TokenType::FunctionCall =>
            (SemanticTokenType::FUNCTION, vec![]),
        
        // Keywords - Blocks
        TokenType::ButtonDef | TokenType::EndButtonDef | 
        TokenType::Extension | TokenType::EndExtension |
        TokenType::Stage | TokenType::EndStage =>
            (SemanticTokenType::KEYWORD, vec![]),
        
        // Preprocessor
        TokenType::Define | TokenType::IfDefined | TokenType::IfNotDefined | 
        TokenType::ElseDefined | TokenType::EndIfDefined | TokenType::PreprocessorDisabled =>
            (SemanticTokenType::MACRO, vec![]),
        ...
        // String commands
        TokenType::String | TokenType::StringLn | TokenType::StringBlock | 
        TokenType::StringLnBlock | TokenType::EndString =>
            (SemanticTokenType::KEYWORD, vec![SemanticTokenModifier::DECLARATION]),
        
        // Timing commands
        TokenType::Delay | TokenType::DelayVar | TokenType::DefaultDelay | TokenType::StringDelay =>
            (SemanticTokenType::KEYWORD, vec![]),
        
        // Variables
        TokenType::Assignment | TokenType::Declaration =>
            (SemanticTokenType::VARIABLE, vec![SemanticTokenModifier::DECLARATION]),
        
        // System commands
        TokenType::Attackmode | TokenType::InjectVar | TokenType::Inject | 
        TokenType::InjectMod | TokenType::Keycode | TokenType::ExfilVar =>
            (SemanticTokenType::KEYWORD, vec![SemanticTokenModifier::MODIFICATION]),
        
        // Key commands
        TokenType::Hold | TokenType::Release | TokenType::Repeat | TokenType::Enter |
        TokenType::ModKeyDown | TokenType::ModKeyUp | TokenType::KeyDown | 
        TokenType::KeyUp | TokenType::ModDown | TokenType::ModUp =>
            (SemanticTokenType::KEYWORD, vec![]),
        
        // Modifier keys
        TokenType::Modifier =>
            (SemanticTokenType::new("modifier"), vec![]),
        
        // Random generators
        TokenType::RandomLowercaseLetter | TokenType::RandomUppercaseLetter |
        TokenType::RandomNumber | TokenType::RandomLetter | 
        TokenType::RandomSpecial | TokenType::RandomChar =>
            (SemanticTokenType::FUNCTION, vec![SemanticTokenModifier::STATIC]),
        ...
        // Unknown
        TokenType::Unknown =>
            (SemanticTokenType::new("unknown"), vec![]),
    }
}
...
pub fn generate_semantic_tokens(content: &str) -> Vec<SemanticToken> {
    ...
    for (line_idx, line) in content.lines().enumerate() {
        ...
        let (semantic_type, modifiers) = if in_rem_block && token_type != TokenType::RemBlock && token_type != TokenType::EndRemBlock {
            (SemanticTokenType::COMMENT, vec![])
        } else {
        ...
        let keyword_length = if in_rem_block || matches!(token_type, TokenType::Rem | TokenType::RemBlock | TokenType::EndRemBlock | TokenType::Comment) {
            token_text.len()
        ...
        if !in_rem_block && !matches!(token_type, TokenType::Rem | TokenType::RemBlock | TokenType::EndRemBlock | TokenType::Comment) {
            ...
            for (i, ch) in rest_of_line.chars().enumerate() {
                ...
                if ch == '#' {
                    let const_name: String = rest_of_line[i..]
                        .chars()
                ...
                // Variable detection ($var)
                    ...
                    let var_name: String = rest_of_line[i..]
                        .chars()
                        .take_while(|c| c.is_alphanumeric() || *c == '_' || *c == '$')
                        .collect();
            ...
            // Number detection
            else if ch.is_ascii_digit() && (i == 0 || !rest_of_line.chars().nth(i-1).unwrap_or(' ').is_alphanumeric()) {
                let number: String = rest_of_line[i..]
                    .chars()
                    .take_while(|c| c.is_ascii_digit() || *c == 'x' || (*c >= 'a' && *c <= 'f') || (*c >= 'A' && *c <= 'F'))
                    .collect();
            ...
            // Boolean detection (TRUE/FALSE)
                ...
                let word: String = rest_of_line[i..]
                    .chars()
                ...
                if word == "TRUE" || word == "FALSE" {
                    ...
                    let is_word_boundary = i == 0 || !rest_of_line.chars().nth(i-1).unwrap_or(' ').is_alphanumeric();
                    let next_pos = i + word.len();
                    let is_end_boundary = next_pos >= rest_of_line.len() || !rest_of_line.chars().nth(next_pos).unwrap_or(' ').is_alphanumeric();
                    ...
                    if is_word_boundary && is_end_boundary {
                        ...
                        tokens.push(SemanticToken {
                            ...
                            token_type: semantic_type_to_index(&SemanticTokenType::new("boolean")),
                            token_modifiers_bitset: 0,
                        });
