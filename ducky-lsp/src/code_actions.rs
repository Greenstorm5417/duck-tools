...
pub fn get_code_actions(content: &str, range: Range, diagnostics: Vec<Diagnostic>, uri: &Url) -> Vec<CodeAction> {
...
fn find_unused_variables(content: &str) -> Vec<(u32, String)> {
    ...
    for (line_idx, line) in lines.iter().enumerate() {
        ...
        if trimmed.starts_with("VAR ") {
            ...
            if let Some(var_name) = trimmed.strip_prefix("VAR ").and_then(|s| s.split_whitespace().next()) {
